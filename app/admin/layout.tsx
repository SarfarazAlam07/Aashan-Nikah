...
export default function AdminLayout({
  ...
  useEffect(() => {
    if (!authLoading) {
      ...
      // ✅ Fix: Allow SUPER_ADMIN only
      if (user?.role !== 'SUPER_ADMIN') {
        toast.error('Unauthorized access - Admin only');
        if (user?.role === 'MUFTI') {
          setTimeout(() => router.push('/mufti/dashboard'), 1000);
        } else {
  ...
  // ✅ Admin Navigation Items (Super Admin only) - Settings removed
  const navItems = [
    ...
    // { href: '/admin/settings', icon: FiSettings, label: 'Settings' }, ❌ Removed
  ];
        ...
        <div className="mx-4 mt-4 p-4 bg-gradient-to-br from-amber-500/10 to-amber-600/5 rounded-xl border border-amber-500/20">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="inline-block text-[10px] px-2 py-0.5 rounded-full mt-1 bg-amber-500/20 text-amber-400">
                Super Admin
              </span>
            </div>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {active && <FiChevronRight size={14} className="text-amber-400" />}
              </Link>
            );
          })}
        </nav>
